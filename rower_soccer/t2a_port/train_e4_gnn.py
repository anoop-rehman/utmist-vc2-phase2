"""D3 M3 E4: alternating-snapshot self-play for Transform2Act on run-to-goal.

Derived from `train_e3_gnn.py`, which is left untouched so every E3/E3.1 number
stays reproducible. The learner's loop is unchanged; what is added is the
snapshot exchange (`e4_selfplay.py`) and the two race guards.

WHY THIS RUNG. `parse`'s sign has never been in play. E3.1's winners finished
in 76-131 steps against a scripted opponent that arrived at 491, so the coupled
+/-1000 term was a near-constant +1000 for the whole experiment. Here both
sides are real racers. At alpha 0.847 the win->loss swing is 306 points against
a dense component of 376 -- 58% of the weighted buffer return.

Both lineages train as AGENT 0 and face the partner's snapshot in slot 1, which
is slot 0 rotated by pi about z. See `design_opt/envs/run_to_goal_sp.py` for
the transform and `gate_e4.py` for the eleven assertions that must pass before
...
def main():
    ...
    p.add_argument("--partner-cfg", required=True,
                   help="the other lineage's cfg id, e.g. rtg_e4_s1b")
    ...
    L(f"E4 GNN self-play: cfg {args.cfg} seed {cfg.seed} threads {args.num_threads} "
      ...
      f"opponent_mode {env.opp_mode} partner {args.partner_cfg} "
    ...
    def refresh_opponent(epoch):
        """Publish ours, then install the partner's. In that order, so the
        pair self-synchronises without a barrier."""
        # to_cpu: the env hands the policy CPU tensors, the learner lives on
        # cuda. to_test: eval mode, so publishing cannot update the policy's
        # own running-norm statistics -- a probe must not perturb the run it
        # measures. Same wrapper the per-epoch morphology census uses.
        with to_cpu(agent.policy_net), to_test(agent.policy_net):
            body = sp.dump_mean_action_body(env, agent.policy_net, e3_morph)
        if body:
            sp.publish(args.snapshot_root, args.cfg, epoch,
        ...
        if got is None:
            ...
            return False
        pol_dict, body_xml, meta = got
        if meta.get("epoch") == opp_meta["epoch"] and opp_meta["cfg"]:
            L(f"  [e4] epoch {epoch}: partner still at epoch "
              f"{meta.get('epoch')}; opponent unchanged")
            return False
        with e3_morph.rng_guard(env):
            sp.install_opponent(env, cfg, pol_dict, body_xml, merged_out,
                                rtg_scene.build, BASE_SRC, make_policy)
        opp_meta.update(epoch=meta.get("epoch"), cfg=meta.get("cfg"))
        L(f"  [e4] epoch {epoch}: opponent <- {args.partner_cfg} "
          f"@epoch {meta.get('epoch')} ({meta.get('n_bodies')} bodies)")
        return True

    recent_evals = []
    for epoch in range(start_epoch, cfg.max_epoch_num):
        if args.opp_refresh and epoch % args.opp_refresh == 0:
            t_ref = time.time()
            swapped = refresh_opponent(epoch)
            if swapped:
                L(f"  [e4] refresh took {time.time() - t_ref:.1f}s")
        ...
        payload = {
            "e4/train_R_STOCHASTIC": float(log.avg_reward),
            "e4/train_R_eps_STOCHASTIC": float(log.avg_episode_reward),
            "e4/exec_R_MEANACTION_eval": float(log_eval.avg_exec_reward),
            "e4/exec_R_eps_MEANACTION_eval":
                float(log_eval.avg_exec_episode_reward),
            "e4/ep_len": float(log.avg_episode_len),
            "e4/num_episodes": int(log.num_episodes),
            "e4/total_steps": total_steps,
            "e4/T_sample": info["T_sample"], "e4/T_update": info["T_update"],
            "e4/T_eval": info["T_eval"],
        }
        if agent.cur_alpha is not None:
            payload["e4/alpha"] = float(agent.cur_alpha)
            ...
            payload["e4/dodge_worth"] = float((1.0 - agent.cur_alpha) * 1000.0)
        ...
        if args.eval_every and (epoch + 1) % args.eval_every == 0:
            ...
            eps = ev.pop("episodes", [])
            payload.update({f"e4/eval_{k}": v for k, v in ev.items()})
