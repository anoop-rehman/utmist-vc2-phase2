...
"""Task for a walker to accelerate towards a target based off of dm_control task go_to_target.py."""
...
DEFAULT_ACCLERATION_TOLERANCE_TO_TARGET = 1.0

class AccelForward(composer.Task):
  """A task that requires a walker to accelerate forwards."""
  ...
  def __init__(self,
            ...
            #    distance_tolerance=DEFAULT_DISTANCE_TOLERANCE_TO_TARGET,
               acceleration_tolerance=DEFAULT_ACCLERATION_TOLERANCE_TO_TARGET,
    ...
    Args:
      ...
      distance_tolerance: Accepted distance to the target position before
        providing reward.
      acceleration_tolerance: Accepted accleration speed to target position 
        before providing reward. 
    ...
    self._acceleration_tolerance = acceleration_tolerance
...
#     return np.array(physics.bind(self._target).pos)
  ...
  def get_reward(self, physics):
    ...
    distance = np.linalg.norm(
      physics.bind(self._walker_spawn_position).pos[:2] - 
      physics.bind(self._walker.root_body).xpos[:2])
    timestep = physics.timestep()
    acceleration = distance / (timestep ** 2)
    if acceleration > self._acceleration_tolerance: 
        reward = 1.
