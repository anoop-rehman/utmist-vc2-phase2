...
class SVG0(nn.Module):
    ...
    def optimize(self, batch, global_step):
        ...
        # print("pi:", pi)
        # print("Type:", type(pi))
        # print("Shape:", pi.shape if hasattr(pi, 'shape') else 'N/A')

        # # Convert pi to a NumPy array if it's a tensor
        # if isinstance(pi, torch.Tensor):
        #     pi_numpy = pi.cpu().detach().numpy()
        #     writer.add_histogram("SVG0/policy_histogram", pi_numpy, global_step)
        # else:
        #     # Handle the case where pi is not a PyTorch tensor (unlikely in this context but for completeness)
