...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action):
        ...
        # Update velocities list and calculate moving average
        self.x_velocities.append(x_velocity)
        if len(self.x_velocities) > 100:
            self.x_velocities.pop(0)  # Remove the oldest entry if there are more than 100

        # Calculate the moving average of x velocities
        self.averaged_x_velocity = np.mean(self.x_velocities)
    
        forward_reward = 1/((self.averaged_x_velocity - self.target_x_velocity)**2 + 0.000001)
        ...
        print("--------------------")
        print('forward_reward:', info["forward_reward"])
